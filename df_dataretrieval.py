...
class IndexData_Retrieval:
    ...
    Parameters:
        filename (str): Name of the pickle file to save or load the dataframe.
        link (str): URL link to a Wikipedia page containing stock exchange information.
        frequency (str): Time interval of historical data to load (e.g., '1min', '1day', '1W').
    ...
    def get_stockex_tickers(self):
        # Read tables from Wikipedia link to obtain tickers
        tables = pd.read_html(self.link)
        df = tables[4]  # Assuming the target table is at index 4
        # Drop irrelevant columns for ticker information
    ...
    def fetch_data(self, start_date: datetime, end_date: datetime):
        if self.use_yfinance:
            # Check if selected frequency is valid for yfinance
